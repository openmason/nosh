...
var _help = [
  ...
  'reset request/result/all ' +  'resets request|result object'.grey,
...
var _commands = 'headers verbose auth request result reset output filter get post put head delete read'.split(' ');
...
function RestCli(config) {
  ...
  if(config) {
    ...
    this.userprompt = config.prompt || this.ps1;
    this.ps1 = this.userprompt;
...
RestCli.prototype.setPrompt = function() {
  var urlPath = url.parse(this.url);
  if(urlPath) {
    this.ps1 = this.userprompt || urlPath.protocol;
...
// url manipulations
RestCli.prototype.setUrlToRoot = function() {
  var urlPath = url.parse(this.url);
  if(urlPath) {
    urlPath.pathname='/';
    this.url=url.format(urlPath);
  }
  this.setPrompt();
};
RestCli.prototype.setUrlToParent = function() {
  var urlPath = url.parse(this.url);
  if(urlPath) {
    var segments = urlPath.pathname.split('/');
    var parentPath=segments.length>0&&segments.slice(0,segments.length-1).join('/') || '/';
    urlPath.pathname=parentPath==''?'/':parentPath;
    this.url=url.format(urlPath);
  }
  this.setPrompt();
};
...
// resets to defaults
RestCli.prototype.reset = function() {
  this.cookies = {};
  this.headers = { 'Accept':'*/*' };
...
/* ------------ symbols and values ------------- */
RestCli.prototype.setRequestObj = function (robj) {
  this.requestObj = robj;
  panda.symbols()['request']=this.requestObj;
};

RestCli.prototype.setResultObj = function (robj) {
  this.resultObj = robj;
  panda.symbols()['result']=this.resultObj;
};
...
RestCli.prototype.dispatch = function(command, nosh, done) {
  ...
  if(/^(HEADERS)/i.test(command)) {
    ...
    if(argv._.length>1) {
      var arg = argv._[1];
      if(arg=='request') {
        self.setRequestObj(undefined);
      } else if(arg=='result') {
        self.setResultObj({});
      } else if(arg=='all') {
        this.reset();
      } else {
        util.puts("invalid argument: " + arg.red);
      }
      done();
      return;
    }
    util.puts("don't know what to reset? see help");
    done();
    ...
    if(argv._.length>1) {
      ...
      if(fileContents) {
        self.setRequestObj(fileContents);
    ...
    nosh.ask(["key", "value"], function(answer) {
      if(answer && answer.key) {
        ...
        self.setRequestObj(self.requestObj);
        util.puts(JSON.stringify(self.requestObj));
      }
      done();
    });
