...
Nosh.prototype.completer = function completer(line) {
  line = line.trim();
  var tabCompletions = this.completions.concat(Object.keys(panda.symbols()));
...
Nosh.prototype.exec = function(cmd) {
  ...
  try {
    ...
    // looks like parsing went thru fine
    // lets simply load it
    panda.execute(ast);
    return self.prompt();
  } catch(err) {
    // lets try to see if the lookup works
    var m = panda.lookup(cmd);
    if(m) {
      // if m is a list, need to take care of that
      if(handy.getType(m)=='array') {
        async.forEachSeries(m, function(cmd, nxt) { 
          self.dispatch(cmd, nxt);
        }, function(err) {
          return self.prompt();
        });
        return;
...
Nosh.prototype.dispatch = function(command, done) {
  ...
  if(/^(.QUIT)/i.test(command)) {
    ...
    var aliases = panda.symbols();
    for(var symbol in aliases) {
      console.log(symbol.cyan, '\t', JSON.stringify(aliases[symbol]).grey);
    }
    done();
  } else {
