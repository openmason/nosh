/**
 * nosh - copyright(c) 2012,2013 openmason.
 * MIT Licensed
 */
...
function Nosh(config) {
  ...
  var module = (config.application && config.application.start) || 
    (loadableModules.length>0 && loadableModules[0]) || 'unknown';
...
Nosh.prototype.extractWords = function(command) {
  ...
  var words = (command && command.match(/[^\s]+|"[^"]+"/g)) || [];
  var n = words.length;
...
Nosh.prototype.dispatch = function(command, done) {
  ...
  } else if(/^(LET )/i.test(command)) {
    // let command would copy a panda symbol
    // should be of the form 
    // let a=b
    var tmpCmd=command.substr(4);
    var kv=tmpCmd.split(/=/);
    if(kv && kv.length==2) {
      panda.symbols()[kv[0]] = panda.symbols()[kv[1]];
      util.puts('key \''+kv[0]+'\' set to \''+panda.symbols()[kv[0]]+'\'');
    }
    // else, do nothing just ignore, let the user see help
    done();
