...
Nosh.prototype.version = fs.readFileSync(require('path').join(__dirname, '..', 'package.json'))
...
Nosh.prototype.completer = function completer(line, fn) {
...
Nosh.prototype.start = function(config) {
...
Nosh.prototype.dispatch = function(command, done) {
  ...
  if(/^(GET|POST|PUT|HEAD|DELETE)/i.test(command)) {
    ...
    var options = { method: method, username:username, password:password };
